...
In the `<body>` of the HTML file, there could be three special custom elements: `<notty-transclusion target="notty:..." show-metadata="..." expanded="..." disable-numbering="..." demote-headings="..."></notty-transclusion>`, `<notty-internal-link target="notty:...">...</notty-internal-link>` and `<notty-cite target="notty:..."></notty-cite>`. 
`<notty-transclusion>` is used to represent transcluded notes, `<notty-internal-link>` is used for internal links between notes, and `<notty-cite>` is used for citations to notes, which is basically a special kind of internal link.
For `<notty-transclusion>`, its body must be empty; the `target` attribute, starting with `notty:`, specifies the ID (not including the `notty:` prefix) of the note to be transcluded, while `show-metadata` and `expanded` are boolean attributes that control the display of metadata and whether the transclusion is expanded by default, respectively; due to limitations in Typst's HTML export capabilities, their values are represented as strings ("true" or "false"). For `<notty-internal-link>`, the `target` attribute specifies the ID of the note to link to, and its body contains the link text. For `<notty-cite>`, the `target` attribute specifies the ID of the note to cite, and its body contains the citation text.

Then, Notty extracts information from the generated HTML files, and use the Tera templating engine and user-supplied templates to produce the final HTML files for the notes. By default, Notty looks for templates in `.notty/templates/`. 

This rendering process begins by parsing the HTML files to build a transclusion graph with respect to the `<notty-transclusion>` elements. Then, the notes are processed in topological order. For each note, the aforementioned custom elements are replaced with the actual content they represent.

First, the transclusion and linking relationships are analyzed to build a transclusion graph. Each note is represented as a node in the graph, and a directed edge from node A to node B exists if note A transcludes note B. If there are cycles in the transclusion graph, Notty will report an error and abort the rendering process, as cyclic transclusions are not supported.

Then, transclusions are processed. For `<notty-transclusion>`, it is rendered via the `transclusion.html` template, which is provided with a a `transclusion` context (`transclusion.target`, `transclusion.show_metadata`, `transclusion.expanded`, `transclusion.hide_numbering`, `transclusion.demote_headings`, `transclusion.content`). The `transclusion.target`, `transclusion.show_metadata`, `transclusion.expanded`, `transclusion.hide_numbering`, and `transclusion.demote_headings` are extracted from the corresponding attributes of the `<notty-transclusion>` element, while `transclusion.content` is the processed content of the target note's final HTML file, to help simplify transclusion rendering in templates. Two Tera filters are registered to help transclusion rendering: `notty_hide_numbering` and `notty_demote_headings`. They apply unconditionally; template conditionals decide whether to invoke them (see the default `transclusion.html`). The result of rendering this template replaces the corresponding `<notty-transclusion>` element in the final HTML file. By processing the notes in topological order, the target note should have already been processed when processing the current note. After this step, there should be only `<notty-internal-link>` and `<notty-cite>` elements left in the HTML file.

For `<notty-internal-link>`, it is rendered via the `internal_link.html` template, which is provided with a `link` context (`link.target`, `link.text`, `link.href`). The `link.target` and `link.text` are extracted from the corresponding attributes and body of the `<notty-internal-link>` element, while `link.href` is the generated URL to the target note's final HTML file, to help simplify link generation in templates. The result of rendering this template replaces the corresponding `<notty-internal-link>` element in the final HTML file. The rendering process for `<notty-cite>` is similar, except that it uses the `citation.html` template and a `citation` context (`citation.target`, `citation.text`, `citation.href`).

Then, backmatters are generated for each note. As for now, Notty supports four types of backmatter sections: contexts, references, backlinks, and related notes.
