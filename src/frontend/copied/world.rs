use std::{collections::HashMap, fmt, fs, io::{self, Read}, mem, path::{Path, PathBuf}, sync::{LazyLock, OnceLock}};

use chrono::{DateTime, Datelike, FixedOffset, Local, Utc};
use parking_lot::Mutex;
use typst::{diag::{eco_format, EcoString, FileError, FileResult}, foundations::{Bytes, Dict, IntoValue}, text::{Font, FontBook}, Feature, Features, Library, World};
use typst_kit::{fonts::{FontSlot, Fonts}, package::PackageStorage};
use typst_library::foundations::Datetime;
use typst_syntax::{FileId, Source, VirtualPath};
use typst_utils::{LazyHash};

use crate::{args::{Input, ProcessArgs, WorldArgs}, frontend::copied::download::PrintDownload};
...
impl SystemWorld {
    ...
    pub fn new(
        ...
        let library = {
            ...
            let features = Features::from_iter([Feature::Html]);
        ...
        Ok(Self {
            ...
            package_storage: crate::frontend::copied::package::storage(&world_args.package),
            now,
        })
    }
    ...
    /// Set a new main source file.
    ///
    /// The given `path` is resolved relative to the operating system and must
    /// reside within this world's `root`. If the file was already known, the
    /// existing `FileId` is reused, otherwise it is inserted into the slot map.
    /// After this call, [`SystemWorld::main`] returns the newly assigned id.
    pub fn set_main<P: AsRef<Path>>(
        &mut self,
        path: P,
    ) -> Result<(), WorldCreationError> {
        let path = path.as_ref();

        // Canonicalize the provided path so that it can be compared reliably.
        let canonical = path.canonicalize().map_err(|err| match err.kind() {
            io::ErrorKind::NotFound => {
                WorldCreationError::InputNotFound(path.to_path_buf())
            }
            _ => WorldCreationError::Io(err),
        })?;

        // Convert to a virtual path relative to the project root.
        let vpath = VirtualPath::within_root(&canonical, &self.root)
            .ok_or(WorldCreationError::InputOutsideRoot)?;

        // Obtain or create the corresponding FileId.
        let id = FileId::new(None, vpath);

        // Ensure a slot exists for this id so it can be accessed later.
        {
            let mut slots = self.slots.lock();
            slots.entry(id).or_insert_with(|| FileSlot::new(id));
        }

        // Finally, update `self.main`.
        self.main = id;
        Ok(())
    }
